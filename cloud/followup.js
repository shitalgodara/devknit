/*
input nothing or date of requested data
output user parse objects with input username,phone,name,created_groups
function to get yesterdays signups
...
/*
input name,phone,status,person,date,remarks
output bool
...
/*
input nothing
output todays followup parse objects with feilds name,phone,status,person,date,remarks
...
query.find({
  success: function(results) {
    for(var i=0;i<results.length;i++){
      ...
      if(temp_array[3]==1){temp_array[4]=results[i].createdAt;}
      h.setItem(item,temp_array);
    }
...
temp_array[3]=temp_array[3]+h2.getItem(temp_array[5][j]);
    ...
    //console.log('key is: ' + keys[i] + ', value is: ' + h.getItem(keys[i]));
}
//response.success(results);
response.success(h);
  },
...
//trying background job
//whole 8 user state dfa->user where drop rates and all user
//complete analytics
...
query.find({
 ...
 for(var i=0;i<results.length;i++){
      ...
      if(temp_array[0]==1){temp_array[1]=results[i].createdAt;}
      h.setItem(item,temp_array);
    }
        var a=new Array(5);
    a[0]=0;
...
query.find({
 ...
 mm=mm+msgs;
    if(msgs==0){h.setItem("drop_01");a[0]++;}
      else if((msgs<50)&&(lastdate>=one_month_ago)){h.setItem("User");a[1]++;}
        else if((msgs<50)&&(lastdate<one_month_ago)){h.setItem("drop_2");a[2]++;}
          else if((msgs<=50)&&(lastdate>=one_month_ago)){h.setItem("Retened");a[3]++;}
            else if((msgs<=50)&&(lastdate<one_month_ago)){h.setItem("drop_3");a[4]++;}
...
query.find({
 ...
 mm=mm+msgs;
    ...
    if(msgs==0){h.setItem("drop_01");a[0]++;}
      else if((msgs<20)&&(lastdate>=four_month_ago)){h.setItem("User");a[1]++;console.log(keys[i]);ids.push(keys[i]);}
        else if((msgs<20)&&(lastdate<four_month_ago)){h.setItem("drop_2");a[2]++;}
          else if((msgs>=20)&&(lastdate>=four_month_ago)){h.setItem("Retened");a[3]++;}
            else if((msgs>=20)&&(lastdate<four_month_ago)){h.setItem("drop_3");a[4]++;}
            else{a[5]++; console.log(lastdate+"PPP"+msgs);}
